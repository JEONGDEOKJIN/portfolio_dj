...
const CarouselArchitectureMonami = () => {
  const [...projectMonami] = dummyMonamiProject;
  // console.log("projectName", projectMonami[0].projectName); // 🔵 정상적으로 나옴, 첫번째 배열이 그대로 잘 나오기
  // console.log("projectName", projectMonami[0].architectureImageHref); // 🔵 정상적으로 나옴, 첫번째 배열이 그대로 잘 나오기
  ...
  return (
    <article className="text-[100px] z-20 text-white w-full h-full  flex items-center justify-center">
      <Carousel className="w-full max-w-[70%]  ">
        <CarouselContent>
          {projectMonami[0].architectureImageHref.map((item, index) => (
...
export default CarouselArchitectureMonami;
